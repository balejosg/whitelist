...
function initMainListeners() {
    ...
    document.getElementById('repo-config-form')?.addEventListener('submit', async (e) => {
        ...
        const gruposDirInput = document.getElementById('config-grupos-dir');
        ...
        try {
            ...
            if (userEl && state.currentUser)
                userEl.textContent = state.currentUser.login || '';
            updateEditUI();
    ...
    document.querySelectorAll('.tab').forEach(tab => {
        tab.addEventListener('click', () => {
            const rawType = tab.dataset.type;
            // Map singular to plural keys for state
            let type = 'whitelist';
            if (rawType === 'blocked_subdomain')
                type = 'blocked_subdomains';
    ...
    document.getElementById('save-config-btn')?.addEventListener('click', async () => {
        if (!state.canEdit || !state.currentGroupData)
            return;
    ...
    document.getElementById('add-rule-form')?.addEventListener('submit', async (e) => {
        ...
        if (!state.canEdit || !state.currentGroupData)
            return;
        const input = document.getElementById('new-rule-value');
    ...
    document.getElementById('bulk-add-form')?.addEventListener('submit', async (e) => {
        ...
        if (!state.canEdit || !state.currentGroupData)
            return;
        const input = document.getElementById('bulk-values');
        ...
        let added = 0;
        // Ensure array exists
        if (!state.currentGroupData[typeKey])
            state.currentGroupData[typeKey] = [];

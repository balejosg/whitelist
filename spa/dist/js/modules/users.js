...
window.revokeRole = async (userId, roleArg) => {
...
window.openAssignRoleModal = (userId, userName) => {
    const idInput = document.getElementById('assign-role-user-id');
    ...
    if (select) {
        select.innerHTML = state.allGroups.map(g => `<option value="${escapeHtml(g.name)}">${escapeHtml(g.name)}</option>`).join('');
    }

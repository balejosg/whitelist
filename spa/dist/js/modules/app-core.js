...
export async function init() {
    ...
    if (callbackResult?.error) {
        ...
        if (errorEl)
            errorEl.textContent = 'Authentication error: ' + callbackResult.error;
        return;
    }
    ...
    if (isGitHubLoggedIn && config.owner && config.repo) {
        ...
        if (token) {
            const api = new GitHubAPI(token, config.owner, config.repo, config.branch || 'main');
            setGithub(api);
...
export function updateEditUI() {
    ...
    const editButtons = document.querySelectorAll('#new-group-btn, #save-config-btn, #delete-group-btn, #add-rule-btn, #bulk-add-btn');
    ...
    if (teacherBanner) {
        if (isTeacher && !isAdmin) {
            ...
            if (groupsEl)
                groupsEl.textContent = groups.length > 0 ? groups.join(', ') : 'no groups yet';
        }
        else {
...
async function initScheduleSection() {
    ...
    select.addEventListener('change', async () => {
        ...
        if (classroomId && SchedulesModule) {
            ...
            if (container)
                container.innerHTML = '<p class="empty-message">Select a classroom to view its schedule</p>';
        }
    });
    // Refresh button

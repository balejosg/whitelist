 ...
 */
import type { DomainRequest, RequestStatus, RequestPriority, User, SafeUser, Role, UserRole, Classroom, Machine, Schedule, PushSubscription } from './index.js';
/**
...
export interface IRequestStorage {
    ...
    updateRequestStatus(id: string, status: 'approved' | 'rejected', resolvedBy?: string, note?: string | null): DomainRequest | null;
...
export interface IScheduleStorage {
    ...
    checkConflict(classroomId: string, dayOfWeek: number, startTime: string, endTime: string, excludeId?: string): ScheduleConflict | null;
}
/**
...
export interface IGitHubService {
    ...
    updateWhitelistFile(groupId: string, domains: string[], message: string): Promise<boolean>;
    isDomainInWhitelist(groupId: string, domain: string): Promise<boolean>;
    addDomainToWhitelist(groupId: string, domain: string, message?: string): Promise<boolean>;
